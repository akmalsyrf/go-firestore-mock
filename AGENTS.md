...
5. **Do not add `t.Skip` in `fstest`.** Skips fail the accuracy gate. Prefer a waiver if the emulator cannot support the feature.
...
3. If cursors/snapshots are involved, unwrap with `Reference()` / `unwrapCursorArgs`.
4. `make generate`
